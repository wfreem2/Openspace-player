...
export class CreatorMenuItemComponent extends BaseComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		this.formattedHotKeys = this.menuItem.hotKey.reduce((prev, curr) => prev + '+' + curr, OsService.getCommandKey)

		if (this.menuItem.isDisabled instanceof Observable) {
			this.menuItem.isDisabled.pipe(takeUntil(this.$unsub)).subscribe((val) => (this.isDisabled = val))
		} else {
