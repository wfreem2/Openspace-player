...
describe('CreateComponent', () => {
	...
	it("selecting a scene should set the current forms values to the scene's", (done) => {
		...
		let id = 100
		component.$selectedScene.pipe(first()).subscribe((scene) => {
	...
	it('#onDeleteClicked() should show confirmation popup', (done) => {
		...
		component.$isConfirmShowing.pipe(first()).subscribe((isConfirmShowing) => {
	...
	it('#deleteScene() should remove scene from list', (done) => {
		...
		component.$selectedScene.pipe(skip(1), first()).subscribe((scene) => {
